 ...
 * _exportCore functions, enabling unified step code between app and exports.
 *
 * Run this script:
...
function processExportedVariable(
  ...
  let exportCoreFunctionName: string | null = null;
  ...
  if (name === "_exportCore" && init && ts.isIdentifier(init)) {
    exportCoreFunctionName = init.text;
    result.hasExportCore = true;
  }

  return exportCoreFunctionName;
}
...
function processVariableStatement(
  ...
  for (const decl of node.declarationList.declarations) {
    const funcName = processExportedVariable(decl, result);
    if (funcName) {
      exportCoreFunctionName = funcName;
    }
  }
  return exportCoreFunctionName;
}
...
function processNode(
  ...
  if (ts.isVariableStatement(node)) {
    return processVariableStatement(node, result);
  }
...
/** Find the core function in the AST */
function findCoreFunction(
  sourceFile: ts.SourceFile,
  sourceCode: string,
  targetName: string
): StepFileAnalysis["coreFunction"] {
  let coreFunction: StepFileAnalysis["coreFunction"] = null;

  ts.forEachChild(sourceFile, (node) => {
    if (ts.isFunctionDeclaration(node) && node.name?.text === targetName) {
      coreFunction = extractFunctionInfo(node, sourceCode);
    }
  });
...
function analyzeStepFile(filePath: string): StepFileAnalysis {
  ...
  // First pass: find exports, types, and imports
  ts.forEachChild(sourceFile, (node) => {
    const funcName = processNode(node, sourceCode, result);
    if (funcName) {
      exportCoreFunctionName = funcName;
    }
  });

  // Second pass: find the core function body
  if (exportCoreFunctionName) {
    result.coreFunction = findCoreFunction(
      sourceFile,
      sourceCode,
      exportCoreFunctionName
    );
  }
 ...
 * DO NOT EDIT MANUALLY - your changes will be overwritten!
 *
 * Contains auto-generated codegen templates for steps that export _exportCore.
